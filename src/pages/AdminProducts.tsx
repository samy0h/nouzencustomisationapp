...
export default function AdminProducts() {
  ...
  const loadProducts = () => api.getProducts({ limit: 100, active: true })
    .then(response => setProducts(response.data.products))
    .catch(() => setError('Could not load products. Start the API server first.'));

  useEffect(() => {
    loadProducts();
  }, []);

  const archiveProduct = async (event: MouseEvent, productId: string) => {
    event.preventDefault();
    event.stopPropagation();
    if (!window.confirm('Archive this product? It will disappear from the shop but existing orders stay safe.')) return;

    try {
      await api.deleteAdminProduct(productId);
      await loadProducts();
    } catch (deleteError) {
      setError(deleteError instanceof Error ? deleteError.message : 'Could not archive product.');
    }
  };
    ...
    <main className="admin-page">
      <div className="admin-shell">
        <nav className="admin-top-links">
          ...
          <Link to="/admin/settings" className="admin-back">Settings</Link>
        </nav>
        <header className="admin-header">
          <div>
            <span className="admin-eyebrow">PRODUCT STUDIO</span>
            <h1>Choose a product to edit</h1>
            <p>Manage mockup images, color variants, and printable areas.</p>
          </div>
