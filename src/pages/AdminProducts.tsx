...
export default function AdminProducts() {
  ...
  const [form, setForm] = useState({ name: '', slug: '', description: '', price: '', categoryId: '', type: 'OTHER', color: '', colorHex: '#111111', sizes: ['S'] });
  ...
  const createProduct = async () => {
    ...
    try { await api.createAdminProduct({ name: form.name, slug: form.slug, description: form.description, price: Number(form.price), categoryId: form.categoryId, type: form.type, variants: [{ color: form.color, colorHex: form.colorHex, sizes: form.sizes }] }); setForm({ name: '', slug: '', description: '', price: '', categoryId: '', type: 'OTHER', color: '', colorHex: '#111111', sizes: ['S'] }); setShowCreate(false); setError(''); await loadProducts(); } catch (createError) { setError(createError instanceof Error ? createError.message : 'Could not create product.'); }
  };
